...
<div class="ticker" aria-hidden="true"><div class="ticker-track"><div class="ticker-group"><span>An ordinary evening</span><b>✦</b><span>A reason to remain</span><b>✦</b><span>A life worth choosing</span><b>✦</b></div></div></div>
...
(() => {
  ...
  const tickerSeed = track.firstElementChild.cloneNode(true);
  let tickerWidth = 0;
  function fillTicker() {
    const width = ticker.clientWidth;
    if (!width || width === tickerWidth) return;
    tickerWidth = width;
    const group = tickerSeed.cloneNode(true);
    track.replaceChildren(group);
    // Each half must fill the viewport, including the gap at the seam.
    // Identical halves make translateX(-50%) an exact, seamless repeat.
    while (group.getBoundingClientRect().width < width) {
      group.append(...[...tickerSeed.children].map(node => node.cloneNode(true)));
    }
    track.append(group.cloneNode(true));
    track.style.setProperty('--ticker-duration', `${group.getBoundingClientRect().width / 30}s`);
  }
  fillTicker();
  if ('ResizeObserver' in window) new ResizeObserver(fillTicker).observe(ticker);
  else window.addEventListener('resize', fillTicker);
  if (document.fonts) document.fonts.ready.then(() => { tickerWidth = 0; fillTicker(); });
